 ...
 * 用户原话：「注意组件是几乘几」。
 *
 * ## 为什么是"内容宽 : 照片自己要的高"，而不是写死 16:9 或 1:1
 *
 ...
 * 再按小组件自己的排版链路算出"这张照片想要多高"（[WidgetData.photoFrameHeightDp]）：
 *
 * ```
 * 裁剪框比例 = 图片区内容宽（格子宽 − 左右内边距） : 照片自己想要的高
 * ```
 *
 * 这样裁出来的图比例被**锁死**成同一个形状：同一个组件里几张照片永远同形（点一下换一张不会变形），
 * 而在导入时那个尺寸上，组件里的图片框也正好是这个形状 → 渲染时"窗口 = 整张源图"，
 * 一分不裁。用户后来改过组件尺寸时才会裁到（[PhotoFit.layout] 居中取一块填满，绝不拉伸）。
 *
 * **注意它与"图片框实际有多高"不是一回事**（本轮起）：图片框会吃掉列表下方的全部剩余空间
 * （[ExtrasPlanner]），所以框可能比这条框更高 —— 那种情况下组件里显示的是居中取的一块。
 * 这里刻意**不让裁剪框跟着"框实际有多高"走**：那会变成"在 4×2 上导入的图与在 4×3 上导入的图
 * 形状不同"，点一下换一张就变形，是最难查的一类问题。
 *
 * ## 取不到尺寸时的兜底：2:1（[PhotoCrop.fallback]）
 *
 * 部分启动器不上报尺寸（或上报 0 / 被"强制隐藏"档位压成 0），**以及"还没有任何照片"**时按 2:1。
 * 选 2:1 的三个理由：
 ...
 *     "竖图还能留住上半身"之间；
 *  3. 兜底**只影响裁剪界面里框的形状**，不影响渲染正确性 —— 渲染时按"裁剪后那张图自己的
 *     比例"重新算窗口（[PhotoFit.layout]），所以兜底选得不准最多让用户在裁剪时少留/多留一点边，
 *     不会让显示变形或裁错。这条性质很重要：宁可兜底保守，
