 ...
 * 不够就"保图片、弃句子"。用户看过真机之后给了更干脆的解法：**把每日一句拼进副标题那一行**
 * （`第 N 周 · 接下来 X 节 · 句子`，见 [WidgetData.subtitleText]），于是：
 *  - 句子不再需要一块固定的 22dp 边距，图片框独占列表下方的空间（比之前高 22dp）；
 *  - 底部那条「X 分钟前同步」的页脚也被用户要求整行删掉，那 22dp 一并归了图片框；
 ...
 *
 * - `-1` 强制隐藏：图片区不显示（每日一句同样不显示 —— 见 [WidgetData.quoteShown]）；
 * - `0` 自动：上面的余量规则；
