 ...
 *    （[PhotoFit.layout]：比例一致 → 整张铺满；不一致 → 取中间块，两种都铺满、都不拉伸）。
 *    三个偏好键从此不再被读取（说明写在 `widget/PhotoDisplayMode.kt` 里：
 *    那个文件现在只剩三个常量的空壳，只为一处编译保留，没有任何读写路径）。
 * 5. **框里只画一层**（本轮）：上一版为了让"整张照片一个像素都不被裁"，在照片与框之间的
 *    缝隙里铺了一层"同一张照片缩到 1/8 再放大"的模糊底。用户真机上看到的是**块状色块**
 *    （原话：「图片上下怎么有模糊的像素图片块？其实我觉得也可以适当调大裁剪的大小让这些
 *    模糊色块消失」）→ 整条删掉，改为"宁可多裁一点也要画面干净"，裁量归用户在导入时掌握
 *    （见 [PhotoFit] 的类注释，那段就是防止它被加回来的）。
 *
 * ## 日志是有意留的
 *
...
internal object WidgetExtras {
    ...
    fun apply(context: Context, views: RemoteViews, nextPhoto: PendingIntent) {
        ...
        if (plan.photoVisible) {
            ...
            // 它就是**产出位图的尺寸**：取到的那一块会被缩到这个尺寸铺满整框
            //（产出逐值等于框 → 宿主 fitCenter 是恒等变换，见 PhotoFit 的类注释）。
            ...
            if (shown == null) {
                ...
                //   框=226x72dp 照片比例=3.14:1 需要的框高=72dp 整张(铺满) 取块=720x229@(0,0)
                //   留边=0px(0%) 行数=2 底部零头=0dp ...
                // 三条不变量：
                //  · `留边=0px(0%)` —— 照片区里一条留白都没有（框里**只画一层**铺满的照片，
                //    模糊底那一层已经整条删掉，见 [PhotoFit] 的类注释）；
                //  · `取块=` —— 从源图上取的那一块：比例一致时逐值等于 `源=`（一个像素都不裁），
                //    比例不一致时是源图内部正中那一块（用户拿它核对"我的照片被裁掉了哪一块"）；
                //  · `底部零头=0dp` —— 图片框下方的组件底部也没有留白。
                // 哪一列不对，就去查它对应的规则是不是被改回去了。
                Log.i(
                    TAG,
